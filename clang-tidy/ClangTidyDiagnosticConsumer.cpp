...
void
ClangTidyContext::setDiagnosticsEngine(DiagnosticsEngine *Engine)
{
  DiagEngine = Engine;
}

void
ClangTidyContext::setSourceManager(SourceManager *SourceMgr)
{
  DiagEngine->setSourceManager(SourceMgr);
}

void
ClangTidyContext::setCurrentFile(StringRef File)
{
  CurrentFile = File;
...
void
ClangTidyContext::setASTContext(ASTContext *Context)
{
  AstContext = Context;
...
void
ClangTidyContext::setToolPtr(ClangTool *tool)
{
  m_tool = tool;
}

ClangTool *
ClangTidyContext::getToolPtr(void)
{
  return m_tool;
}
...
ClangTidyContext::getOptionsForFile(StringRef File) const
{
  ...
  // unset values.
  return ClangTidyOptions::getDefaults().mergeWith(OptionsProvider->getOptions(File));
}

void
ClangTidyContext::setCheckProfileData(ProfileData *P)
{
  Profile = P;
}

bool
ClangTidyContext::isCheckEnabled(StringRef CheckName) const
{
...
bool
ClangTidyContext::treatAsError(StringRef CheckName) const
{
...
void
ClangTidyContext::storeError(const ClangTidyError &Error)
{
  Errors.push_back(Error);
}

StringRef
ClangTidyContext::getCheckName(unsigned DiagnosticID) const
{
    ...
    LastErrorWasIgnored(false)
{
  ...
  Diags = llvm::make_unique<DiagnosticsEngine>(IntrusiveRefCntPtr<DiagnosticIDs>(new DiagnosticIDs),
                                               &*DiagOpts,
                                               this,
...
void
ClangTidyDiagnosticConsumer::finalizeLastError()
{
  if (!Errors.empty())
    {
...
static bool
IsNOLINTFound(StringRef NolintDirectiveText,
              StringRef Line,
              unsigned DiagID,
...
static bool
LineIsMarkedWithNOLINT(SourceManager &SM,
                       SourceLocation Loc,
                       unsigned DiagID,
                       const ClangTidyContext &Context)
{
...
static bool
LineIsMarkedWithNOLINTinMacro(SourceManager &SM,
                              SourceLocation Loc,
                              unsigned DiagID,
                              const ClangTidyContext &Context)
{
...
void
ClangTidyDiagnosticConsumer::HandleDiagnostic(DiagnosticsEngine::Level DiagLevel,
  ...
  if (DiagLevel == DiagnosticsEngine::Note)
  ...
  else
    {
      ...
      StringRef WarningOption =
        Context.DiagEngine->getDiagnosticIDs()->getWarningOptionForDiag(Info.getID());
      ...
      Errors.emplace_back(CheckName,
                          Level,
                          Context.getCurrentBuildDirectory(),
                          IsWarningAsError);
    }

  ClangTidyDiagnosticRenderer Converter(Context.getLangOpts(), &Context.DiagEngine->getDiagnosticOptions(),
                                        Errors.back());
...
void
ClangTidyDiagnosticConsumer::removeIncompatibleErrors(SmallVectorImpl<ClangTidyError> &Errors) const
{
...
namespace
{
  ...
  {
    bool
    operator()(const ClangTidyError &LHS,
               const ClangTidyError &RHS) const
    {
...
void
ClangTidyDiagnosticConsumer::finish()
{
  finalizeLastError();
