...
namespace clang 
{
  namespace tidy 
  {
    namespace pagesjaunes
    {
        ...
        public:
                       ...
                       const MacroArgs *args) override
          {
			...
			m_parent->emitError(m_diag_engine,
					    t.getLocation(),
					    ExecSQLAllocateToFunctionCall::EXEC_SQL_2_FUNC_ERROR_UNSUPPORTED_STRING_CHARSET,
					    &slKind);
                      }
                  }
	  ...
	  generate_req_allow_overwrite(Options.get("Generate-requests-allow-overwrite",
          ...
          generation_report_modification_in_dir(Options.get("Generation-report-modification-in-dir",
                                                            "./"))
      {
        llvm::outs() << "ExecSQLAllocateToFunctionCall::ExecSQLAllocateToFunctionCall(StringRef Name, ClangTidyContext *Context):\n";
	...
	else
	  {
	    ...
	    //           SourceLocation(),
	    //           ExecSQLAllocateToFunctionCall::EXEC_SQL_2_FUNC_ERROR_INVALID_GROUPS_FILE,
	    //           &generation_request_groups);
	  }
      }
	...
	Options.store(Opts, "Generate-requests-allow-overwrite", generate_req_allow_overwrite);
        ...
        llvm::outs() << "ExecSQLAllocateToFunctionCall::storeOptions(ClangTidyOptions::OptionMap &Opts):\n";
        llvm::outs() << "    generate_req_headers = " << (generate_req_headers?"True":"False") << "\n";
        llvm::outs() << "    generate_req_sources = " << (generate_req_sources?"True":"False") << "\n";
        llvm::outs() << "    generate_req_allow_overwrite = " << (generate_req_allow_overwrite?"True":"False") << "\n";
        llvm::outs() << "    generation_directory = '" << generation_directory << "'\n";
	...
	unsigned diag_id;
      ...
      /**
       * ExecSQLAllocateToFunctionCall::findMacroStringLiteralDefAtLine
       *
       ...
       */
      bool
      ExecSQLAllocateToFunctionCall::findMacroStringLiteralDefAtLine(SourceManager &src_mgr,
								    unsigned ln,
	...
	return ret;
      ...
      /*
       * ExecSQLAllocateToFunctionCall::createParamsDef
       *
       ...
       */
      std::string
      ExecSQLAllocateToFunctionCall::createParamsDef(const std::string& type,
      ...
      /*
       * ExecSQLAllocateToFunctionCall::createParamsDeclareSection
       *
       ...
       */
      std::string
      ExecSQLAllocateToFunctionCall::createParamsDeclareSection(const std::string& type,
                                                            const std::string& elemtype,
                                                            const std::string& size,
                                                            const std::string& name,
      ...
      /*
       * ExecSQLAllocateToFunctionCall::createParamsDecl
       *
       ...
       */
      std::string
      ExecSQLAllocateToFunctionCall::createParamsDecl(const std::string& type,
                                                  const std::string& elemtype,
                                                  const std::string& size)
      {
      ...
      /*
       * ExecSQLAllocateToFunctionCall::createParamsCall
       *
       ...
       */
      std::string
      ExecSQLAllocateToFunctionCall::createParamsCall(const std::string& name)
      {
        return clang::tidy::pagesjaunes::createParamsCall(name);
      }
      
      /*
       * ExecSQLAllocateToFunctionCall::createHostVarList
       *
       ...
       */
      std::string
      ExecSQLAllocateToFunctionCall::createHostVarList(const std::string& name, bool isIndicator = false)
      {
        return clang::tidy::pagesjaunes::createHostVarList(name, isIndicator);
      }
      
      /**
       * ExecSQLAllocateToFunctionCall::findSymbolInFunction
       *
       ...
       */
      const VarDecl *
      ExecSQLAllocateToFunctionCall::findSymbolInFunction(std::string& varName, const FunctionDecl *func)
      {
      ...
      /**
       * ExecSQLAllocateToFunctionCall::findDeclInFunction
       *
       ...
       */
      string2_map
      ExecSQLAllocateToFunctionCall::findDeclInFunction(const FunctionDecl *func, const std::string& symName)
      {
        return clang::tidy::pagesjaunes::findDeclInFunction(func, symName);
      }

      /**
       * ExecSQLAllocateToFunctionCall::findCXXRecordMemberInTranslationUnit
       *
       ...
       */
      string2_map
      ExecSQLAllocateToFunctionCall::findCXXRecordMemberInTranslationUnit(const TranslationUnitDecl *transUnit,
      ...
      /**
       * ExecSQLAllocateToFunctionCall::decodeHostVars
       *
       ...
       */
      map_host_vars
      ExecSQLAllocateToFunctionCall::decodeHostVars(const std::string &hostVarsList)
      {
      ...
      {
        outs() << "void ExecSQLAllocateToFunctionCall::check(const MatchFinder::MatchResult &result): ENTRY!\n";
	...
	if (lineNum > 0 && !errOccured)
	  {
	    ...
	    Regex allocReqRe(PAGESJAUNES_REGEX_EXEC_SQL_ALLOCATE_REQ_RE);
	    ...
	    std::string requestArgs;
            ...
            if (allocReqRe.match(comment, &matches))
              {
                std::string reqAllocName = matches[PAGESJAUNES_REGEX_EXEC_SQL_ALLOCATE_REQ_RE_NAME];
                std::string requestExecSql = matches[PAGESJAUNES_REGEX_EXEC_SQL_ALLOCATE_REQ_RE_ALLOCATE];
                requestExecSql.append(" ");
                requestExecSql.append(matches[PAGESJAUNES_REGEX_EXEC_SQL_ALLOCATE_REQ_RE_HOSTVAR]);
                
                // Append first match (action) to function name
                requestFunctionName.append("allocate");
