//===--- ExecSQLFreeToFunctionCall.cpp - clang-tidy ----------------------------===//
//
  ...
  namespace tidy 
  {
    namespace pagesjaunes
    {
      ...
      namespace
      {
        ...
        public:
          explicit GetStringLiteralsDefines(ExecSQLFreeToFunctionCall *parent,
                                            ExecSQLFreeToFunctionCall::source_range_set_t *srset)
            : m_parent(parent),
                       ...
                       const MacroArgs *args) override
          {
			...
			m_parent->emitError(m_diag_engine,
					    t.getLocation(),
					    ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_UNSUPPORTED_STRING_CHARSET,
					    &slKind);
                      }
                  }
                ...
                if (have_literal)
                  {
                    ...
                    ExecSQLFreeToFunctionCall::SourceRangeForStringLiterals *ent
                      = new ExecSQLFreeToFunctionCall::SourceRangeForStringLiterals(range, sr, macroName);
	  ...
	  DiagnosticsEngine &m_diag_engine;
          ExecSQLFreeToFunctionCall::source_range_set_t *m_macrosStringLiteralsPtr;
        };
      } // namespace
      
      /**
       * ExecSQLFreeToFunctionCall constructor
       *
       * @brief Constructor for the ExecSQLFreeToFunctionCall rewriting check
       *
       ...
       */
      ExecSQLFreeToFunctionCall::ExecSQLFreeToFunctionCall(StringRef Name,
	  ...
	  generate_req_allow_overwrite(Options.get("Generate-requests-allow-overwrite",
          ...
          generation_report_modification_in_dir(Options.get("Generation-report-modification-in-dir",
                                                            "./"))
      {
        llvm::outs() << "ExecSQLFreeToFunctionCall::ExecSQLFreeToFunctionCall(StringRef Name, ClangTidyContext *Context):\n";
	...
	else
	  {
	    ...
	    //           SourceLocation(),
	    //           ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_INVALID_GROUPS_FILE,
	    //           &generation_request_groups);
	  }
      }
       ...
       */
      void
      ExecSQLFreeToFunctionCall::onStartOfTranslationUnit()
      {
       ...
       */
      void
      ExecSQLFreeToFunctionCall::onEndOfTranslationUnit()
      {
       ...
       */
      void
      ExecSQLFreeToFunctionCall::storeOptions(ClangTidyOptions::OptionMap &Opts)
      {
	...
	Options.store(Opts, "Generate-requests-allow-overwrite", generate_req_allow_overwrite);
        ...
        llvm::outs() << "ExecSQLFreeToFunctionCall::storeOptions(ClangTidyOptions::OptionMap &Opts):\n";
        llvm::outs() << "    generate_req_headers = " << (generate_req_headers?"True":"False") << "\n";
        llvm::outs() << "    generate_req_sources = " << (generate_req_sources?"True":"False") << "\n";
        llvm::outs() << "    generate_req_allow_overwrite = " << (generate_req_allow_overwrite?"True":"False") << "\n";
        llvm::outs() << "    generation_directory = '" << generation_directory << "'\n";
       ...
       */
      void 
      ExecSQLFreeToFunctionCall::registerMatchers(MatchFinder *Finder) 
      {
	...
	/* with a sqlstm variable declaration */
      ...
      /**
       * ExecSQLFreeToFunctionCall::registerPPCallbacks
       *
       ...
       */
      void ExecSQLFreeToFunctionCall::registerPPCallbacks(CompilerInstance &compiler) {
        compiler
          .getPreprocessor()
      ...
      /*
       * ExecSQLFreeToFunctionCall::emitDiagAndFix
       *
       ...
       */
      std::string
      ExecSQLFreeToFunctionCall::emitDiagAndFix(const SourceLocation& loc_start,
      ...
      /**
       * ExecSQLFreeToFunctionCall::processTemplate
       *
       ...
       */
      bool
      ExecSQLFreeToFunctionCall::processTemplate(const std::string& tmpl,
	...
	return ret;
      ...
      /**
       * ExecSQLFreeToFunctionCall::doRequestSourceGeneration
       *
       ...
       */
      void
      ExecSQLFreeToFunctionCall::doRequestSourceGeneration(DiagnosticsEngine& diag_engine,
	...
	fileName.append(GENERATION_SOURCE_FILENAME_EXTENSION);
	  ...
	  emitError(diag_engine,
		    dummy,
		    ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_SOURCE_CREATE_DIR,
		    &fileName);          
        else if (!generate_req_allow_overwrite &&
                 (stat (fileName.c_str(), &buffer) == 0))
	  emitError(diag_engine,
		    dummy,
		    ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_SOURCE_EXISTS,
		    &fileName);
	// Process template for creating source file
	else if (!processTemplate(tmpl, fileName, values_map))
	  emitError(diag_engine,
		    dummy,
		    ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_SOURCE_GENERATION,
		    &fileName);
      }
      
      /**
       * ExecSQLFreeToFunctionCall::doRequestHeaderGeneration
       *
       ...
       */
      void
      ExecSQLFreeToFunctionCall::doRequestHeaderGeneration(DiagnosticsEngine& diag_engine,
                                                               const std::string& tmpl,
                                                               string2_map& values_map)
      { 
        SourceLocation dummy;
	...
	fileName.append(GENERATION_HEADER_FILENAME_EXTENSION);
	  ...
	  emitError(diag_engine,
		    dummy,
		    ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_HEADER_CREATE_DIR,
		    &fileName);          
        else if (!generate_req_allow_overwrite &&
                 (stat (fileName.c_str(), &buffer) == 0))
	  emitError(diag_engine,
	...
	// Process template for creating header file
	  ...
	  emitError(diag_engine,
		    dummy,
		    ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_HEADER_GENERATION,
		    &fileName);
      }
       ...
       */
      void
      ExecSQLFreeToFunctionCall::emitError(DiagnosticsEngine &diag_engine,
                                               const SourceLocation& err_loc,
                                               enum ExecSQLFreeToFunctionCallErrorKind kind,
                                               const std::string* msgptr)
      {
        ...
        switch (kind)
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_NO_ERROR:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_ACCESS_CHAR_DATA:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_CANT_FIND_COMMENT_START:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
              getCustomDiagID(DiagnosticsEngine::Error,
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_COMMENT_DONT_MATCH:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
              getCustomDiagID(DiagnosticsEngine::Warning,
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_SOURCE_GENERATION:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
	      getCustomDiagID(DiagnosticsEngine::Error,
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_HEADER_GENERATION:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
	      getCustomDiagID(DiagnosticsEngine::Error,
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_SOURCE_EXISTS:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
	      getCustomDiagID(DiagnosticsEngine::Warning,
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_HEADER_EXISTS:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
              getCustomDiagID(DiagnosticsEngine::Warning,
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_SOURCE_CREATE_DIR:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
              getCustomDiagID(DiagnosticsEngine::Error,
            ...
            /** Cannot generate request header file (no location) */
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_HEADER_CREATE_DIR:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
              getCustomDiagID(DiagnosticsEngine::Error,
                              "Couldn't create directory for '%0'!");
            diag_engine.Report(diag_id).AddString(msg);
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_UNSUPPORTED_STRING_CHARSET:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
              getCustomDiagID(DiagnosticsEngine::Error,
          ...
          case ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_INVALID_GROUPS_FILE:
            diag_id = TidyContext->getASTContext()->getDiagnostics().
              getCustomDiagID(DiagnosticsEngine::Error,
      ...
      /**
       * ExecSQLFreeToFunctionCall::findMacroStringLiteralDefAtLine
       *
       ...
       */
      bool
      ExecSQLFreeToFunctionCall::findMacroStringLiteralDefAtLine(SourceManager &src_mgr,
								    unsigned ln,
	...
	return ret;
      ...
      /*
       * ExecSQLFreeToFunctionCall::createParamsDef
       *
       ...
       */
      std::string
      ExecSQLFreeToFunctionCall::createParamsDef(const std::string& type,
      ...
      /*
       * ExecSQLFreeToFunctionCall::createParamsDeclareSection
       *
       ...
       */
      std::string
      ExecSQLFreeToFunctionCall::createParamsDeclareSection(const std::string& type,
                                                            const std::string& elemtype,
                                                            const std::string& size,
                                                            const std::string& name,
      ...
      /*
       * ExecSQLFreeToFunctionCall::createParamsDecl
       *
       ...
       */
      std::string
      ExecSQLFreeToFunctionCall::createParamsDecl(const std::string& type,
                                                  const std::string& elemtype,
                                                  const std::string& size)
      {
      ...
      /*
       * ExecSQLFreeToFunctionCall::createParamsCall
       *
       ...
       */
      std::string
      ExecSQLFreeToFunctionCall::createParamsCall(const std::string& name)
      {
        return clang::tidy::pagesjaunes::createParamsCall(name);
      }
      
      /*
       * ExecSQLFreeToFunctionCall::createHostVarList
       *
       ...
       */
      std::string
      ExecSQLFreeToFunctionCall::createHostVarList(const std::string& name, bool isIndicator = false)
      {
        return clang::tidy::pagesjaunes::createHostVarList(name, isIndicator);
      }
      
      /**
       * ExecSQLFreeToFunctionCall::findSymbolInFunction
       *
       ...
       */
      const VarDecl *
      ExecSQLFreeToFunctionCall::findSymbolInFunction(std::string& varName, const FunctionDecl *func)
      {
      ...
      /**
       * ExecSQLFreeToFunctionCall::findDeclInFunction
       *
       ...
       */
      string2_map
      ExecSQLFreeToFunctionCall::findDeclInFunction(const FunctionDecl *func, const std::string& symName)
      {
        return clang::tidy::pagesjaunes::findDeclInFunction(TidyContext, func, symName);
      }

      /**
       * ExecSQLFreeToFunctionCall::findCXXRecordMemberInTranslationUnit
       *
       ...
       */
      string2_map
      ExecSQLFreeToFunctionCall::findCXXRecordMemberInTranslationUnit(const TranslationUnitDecl *transUnit,
      ...
      /**
       * ExecSQLFreeToFunctionCall::decodeHostVars
       *
       ...
       */
      map_host_vars
      ExecSQLFreeToFunctionCall::decodeHostVars(const std::string &hostVarsList)
      {
       ...
       */
      void
      ExecSQLFreeToFunctionCall::check(const MatchFinder::MatchResult &result) 
      {
        outs() << "void ExecSQLFreeToFunctionCall::check(const MatchFinder::MatchResult &result): ENTRY!\n";
	...
	if (lineNum > 0 && !errOccured)
	  {
	    ...
	    Regex freeReqRe(PAGESJAUNES_REGEX_EXEC_SQL_FREE_REQ_RE);
	    ...
	    std::string requestArgs;
             ...
             * =============================================================================
             */
            if (freeReqRe.match(comment, &matches))
              {
                std::string reqAllocName = matches[PAGESJAUNES_REGEX_EXEC_SQL_FREE_REQ_RE_NAME];
                std::string requestExecSql = matches[PAGESJAUNES_REGEX_EXEC_SQL_FREE_REQ_RE_FREE];
                requestExecSql.append(" ");
                requestExecSql.append(matches[PAGESJAUNES_REGEX_EXEC_SQL_FREE_REQ_RE_HOSTVAR]);
                
                // Append first match (action) to function name
                requestFunctionName.append("free");
              ...
              emitError(diagEngine,
                        comment_loc_start,
                        ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_COMMENT_DONT_MATCH);
          }
        else
          {
            if (errOccured)
              ...
              emitError(diagEngine,
                        loc_start,
                        ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_ACCESS_CHAR_DATA);

            else
              // We didn't found the comment start ???
              emitError(diagEngine,
                        comment_loc_end,
                        ExecSQLFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_CANT_FIND_COMMENT_START);
          }
      }
    } // namespace pagesjaunes
  } // namespace tidy
