//===--- ExecSQLLOBFreeToFunctionCall.cpp - clang-tidy ----------------------------===//
//
  ...
  namespace tidy 
  {
    namespace pagesjaunes
    {
      ...
      namespace {
        ...
        public:
          explicit GetStringLiteralsDefines(ExecSQLLOBFreeToFunctionCall *parent,
                                            ExecSQLLOBFreeToFunctionCall::source_range_set_t *srset)
            : m_parent(parent),
                ...
                if (have_literal)
                  {
                    ...
                    ExecSQLLOBFreeToFunctionCall::SourceRangeForStringLiterals *ent
                      = new ExecSQLLOBFreeToFunctionCall::SourceRangeForStringLiterals(range, sr, macroName);
        ...
        private:
          ExecSQLLOBFreeToFunctionCall *m_parent;
          const SourceManager &m_src_mgr; 
          ExecSQLLOBFreeToFunctionCall::source_range_set_t *m_macrosStringLiteralsPtr;
        };
      } // namespace
      
      /**
       * ExecSQLLOBFreeToFunctionCall constructor
       *
       ...
       */
      void ExecSQLLOBFreeToFunctionCall::onStartOfTranslationUnit()
      {
       ...
       */
      void ExecSQLLOBFreeToFunctionCall::onEndOfTranslationUnit()
      {
      ...
      /**
       * ExecSQLLOBFreeToFunctionCall::registerPPCallbacks
       *
       ...
       */
      void ExecSQLLOBFreeToFunctionCall::registerPPCallbacks(CompilerInstance &compiler) {
        compiler
          .getPreprocessor()
      ...
      /*
       * ExecSQLLOBFreeToFunctionCall::emitDiagAndFix
       *
      ...
      /**
       * ExecSQLLOBFreeToFunctionCall::processTemplate
       *
       ...
       */
      bool
      ExecSQLLOBFreeToFunctionCall::processTemplate(const std::string& tmpl,
      ...
      /**
       * ExecSQLLOBFreeToFunctionCall::doRequestSourceGeneration
       *
       ...
       */
      void
      ExecSQLLOBFreeToFunctionCall::doRequestSourceGeneration(DiagnosticsEngine& diag_engine,
      ...
      /**
       * ExecSQLLOBFreeToFunctionCall::doRequestHeaderGeneration
       *
       ...
       */
      void
      ExecSQLLOBFreeToFunctionCall::doRequestHeaderGeneration(DiagnosticsEngine& diag_engine,
                                                                const std::string& tmpl,
                                                                string2_map& values_map)
      { 
        SourceLocation dummy;
       ...
       */
      void
      ExecSQLLOBFreeToFunctionCall::emitError(DiagnosticsEngine &diag_engine,
                                                const SourceLocation& err_loc,
                                                enum ExecSQLLOBFreeToFunctionCallErrorKind kind,
	...
	unsigned diag_id;
	    ...
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
          ...
          case ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_NO_ERROR:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
          ...
          case ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_ACCESS_CHAR_DATA:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
          ...
          case ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_CANT_FIND_COMMENT_START:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
	      getCustomDiagID(DiagnosticsEngine::Error,
          ...
          case ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_COMMENT_DONT_MATCH:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
	      getCustomDiagID(DiagnosticsEngine::Warning,
          ...
          case ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_SOURCE_GENERATION:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
	      getCustomDiagID(DiagnosticsEngine::Error,
          ...
          case ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_HEADER_GENERATION:
	    diag_id = TidyContext->getASTContext()->getDiagnostics().
	      getCustomDiagID(DiagnosticsEngine::Error,
        ...
        if (lineNum > 0 && !errOccured)
          {
            ...
            Regex lobFreeReqRe("^.*EXEC SQL[ \t]+(LOB|lob)[ \t]+(FREE|free)[ \t]+(TEMPORARY|temporary)[ \t]+:([A-Za-z0-9]+);.*$");
            ...
            if (lobFreeReqRe.match(comment, &matches))
              ...
              emitError(diagEngine,
                        comment_loc_start,
                        ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_COMMENT_DONT_MATCH);
          }
        else
          {
            if (errOccured)
              ...
              emitError(diagEngine,
                        loc_start,
                        ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_ACCESS_CHAR_DATA);

            else
              // We didn't found the comment start ???
              emitError(diagEngine,
                        comment_loc_end,
                        ExecSQLLOBFreeToFunctionCall::EXEC_SQL_2_FUNC_ERROR_CANT_FIND_COMMENT_START);
          }
      }
    } // namespace pagesjaunes
  } // namespace tidy
