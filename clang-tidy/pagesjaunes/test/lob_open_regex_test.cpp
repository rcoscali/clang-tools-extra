//===------- lob_open_regex_test.cpp - clang-tidy ----------------------------===//
//
  ...
  namespace tidy
  {
    ...
    namespace pagesjaunes
    {
      ...
      namespace test
      {
        ...
        LobOpenRegexTest::LobOpenRegexTest()
          : lob_open_re_m(PAGESJAUNES_REGEX_EXEC_SQL_LOB_OPEN_REQ_RE)
        {
        }
        ...
        void
        LobOpenRegexTest::SetUp(void)
        {
        }
        
        void
        LobOpenRegexTest::TearDown(void)
        {
        }
        
        void
        LobOpenRegexTest::PrintTo(const LobOpenRegexTest& lob_open_regex_test, ::std::ostream* os)
        {
        }

        TEST_F(LobOpenRegexTest, RegexMatchingIndicators)
        {
#define REQ0                                                            \
          "EXEC SQL LOB_OPEN :emp_cv;" 
          
          llvm::StringRef req0(REQ0);
          SmallVector<StringRef, 8> matches0;
          bool retbool0 = get_lob_open_re().match(req0, &matches0);
          EXPECT_TRUE(retbool0);
          int retsize0 = matches0.size();
          EXPECT_EQ(retsize0, 4);
          if (retbool0 && retsize0 == 4)
            {
              EXPECT_STREQ(matches0[0].str().c_str(), "EXEC SQL LOB_OPEN :emp_cv;");
              EXPECT_STREQ(matches0[1].str().c_str(), "LOB_OPEN");
...
#define REQ1                                                            \
          ...
          bool retbool1 = get_lob_open_re().match(req1, &matches1);
          ...
          if (retbool1 && retsize1 == 4)
            {
              ...
              EXPECT_STREQ(matches1[1].str().c_str(), "LOB_OPEN");
              EXPECT_STREQ(matches1[2].str().c_str(), ":emp_cv");
              EXPECT_STREQ(matches1[3].str().c_str(), "emp_cv");
            }
        }

        TEST_F(LobOpenRegexTest, RegexMatchingWeirdSyntax)
        {
#define REQWEIRD_0                                                            \
          "EXEC SQL \n  LOB_OPEN : emp_cv ;"
          ...
          SmallVector<StringRef, 8> matches0;
          bool retbool0 = get_lob_open_re().match(reqweird0, &matches0);
          EXPECT_TRUE(retbool0);
          int retsize0 = matches0.size();
          EXPECT_EQ(retsize0, 4);
          if (retbool0 && retsize0 == 4)
...
#define REQWEIRD_1                                                            \
          ...
          SmallVector<StringRef, 8> matches1;
          bool retbool1 = get_lob_open_re().match(reqweird1, &matches1);
          EXPECT_TRUE(retbool1);
          int retsize1 = matches1.size();
          EXPECT_EQ(retsize1, 4);
          if (retbool1 && retsize1 == 4)
...
#define REQWEIRD_2                                                            \
          ...
          bool retbool2 = get_lob_open_re().match(reqweird2, &matches2);
...
#define REQWEIRD_3                                                            \
          ...
          bool retbool3 = get_lob_open_re().match(reqweird3, &matches3);
          ...
          if (retbool3 && retsize3 == 4)
            {
              ...
              EXPECT_STREQ(matches3[1].str().c_str(), "Lob_Open");
